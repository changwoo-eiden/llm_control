...
class GPTAPIController(Node):
    def __init__(self):
        ...
        self.detector.last_detected = False   # 마지막 탐지 결과 저장용
        self.plan_chain = make_plan_chain()
        self.get_logger().info("🧩 Few-shot Plan Prompt Loaded")
    ...
    def run_sequence(self, sequence: list):
        for idx, step in enumerate(sequence, start=1):
            ...
            if act == "goto":
                ...
                self.detector.last_detected = found  # ✅ 결과 저장
                if found:
                    self.get_logger().info(f"[{idx}] ✅ DETECTED: {cls}")
                else:
                    self.get_logger().info(f"[{idx}] 🙅 NOT FOUND: {cls}")
    ...
    # 1️⃣ setup 실행
        ...
        if setup_seq:
            self.get_logger().info("🚀 Setup 단계 실행 시작")
    ...
    # 3️⃣ 분기 선택
        ...
        # case 1️⃣ 일반 action step
            if "action" in step:
                self.run_sequence([step])
        # case 2️⃣ branch 중첩 구조면 재귀 실행
            elif "branches" in step:
                self.get_logger().info("🔁 Nested branch 발견 — 재귀 실행")
                self.run_plan_recursive(step)
            else:
                self.get_logger().warn(f"⚠️ 알 수 없는 step 구조: {step}")
    ...
    def ask_and_run_once(self):
        ...
        if branch_seq:
            ...
            self.get_logger().info(f"🚦 Branch '{branch_name}' 실행 시작")
            for step in branch_seq:
                if "action" in step:
                    self.run_sequence([step])
                elif "branches" in step:
            ...
            self.get_logger().info("⚙️ Branch 시퀀스가 비어 있음.")
