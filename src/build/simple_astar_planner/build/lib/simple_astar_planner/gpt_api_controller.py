...
client = OpenAI(api_key="REMOVED_KEYproj-H6K2SPF92wcyjsbAy_V2JSmE_uy1jidEOTn3F1afyTwQPNTeEkjT2IBmYzVL81XwGk1WGqxvjXT3BlbkFJRzMNN5ZcU1tQ4WcURKpguRavh7b3ZcaEcqs5iCZpWK3eoebIh25jH3NgTqNVPjssuosXEmVBwA")
    ...
    for i, step in enumerate(seq, start=1):
        ...
        if act == "goto":
            if "x" not in step or "y" not in step:
                raise ValueError(f"{i}번째 goto에 x,y가 필요합니다: {step}")
            ...
            "properties": {
                "sequence": {
                    ...
                    "items": {
                        "type": "object",
                        "properties": {
                            "action":   {"type": "string", "enum": ["goto", "photo", "wait"]},
                            "x":        {"type": "number"},
                            "y":        {"type": "number"},
                            "yaw":      {"type": "number"},
                            "duration": {"type": "number"}
                        },
                        "required": ["action", "x", "y", "yaw", "duration"],
