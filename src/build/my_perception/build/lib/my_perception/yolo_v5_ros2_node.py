...
class YoloV5ROS2Node(Node):
    def __init__(self):
        ...
        self.declare_parameter("model_path", "~/yolov5/yolov5s.pt")
        self.declare_parameter("conf_thres", 0.4)
        self.declare_parameter("device", "cpu")  # or "cpu"
        self.declare_parameter("publish_vision_msgs", True)
        self.declare_parameter("class_filter", [])  # e.g., ["apple"]
        ...
        if VISION_AVAILABLE and self.publish_vision_msgs:
            ...
            self.get_logger().warn("vision_msgs not available; skipping vision_msgs output.")

        self.get_logger().info(f"✅ YOLOv5 node started. Subscribing to {self.image_topic}")

    # ----------------------------------------------------
    def image_callback(self, msg: Image):
        """Perform object detection on incoming frames."""
        ...
        self.get_logger().info_throttle(2.0, f"Detections: {len(json_list)} | {fps:.1f} FPS")

        # Optional: show window for local debugging
